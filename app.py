...
def process_received_data(data):
    try:
        ...
        if date1!=None and date2!=None:
            calculate_statistics(date1, date2)
...
def calculate_statistics(start_date, end_date):
    ...
    operator_averages = {operator: count / len(data) for operator, count in operator_counts.items()}
    ...
    network_type_averages = {network_type: count / len(data) for network_type, count in network_type_counts.items()}
    ...
    signal_power_averages = {network_type: power_sum / 100 for network_type, power_sum in signal_power_sums.items()}
    ...
    for item in data:
        if item.snr is not None:
            snr_sums[item.network_type] = snr_sums.get(item.network_type, 0) + item.snr
            snr_counts[item.network_type] = snr_counts.get(item.network_type, 0) + 1
    snr_averages = {network_type: snr_sums[network_type] / snr_counts[network_type] for network_type in snr_sums}
